...
// Additional tests outside of the suite from https://github.com/go-acme/lego/blob/master/providers/dns/dnsimple/dnsimple_test.go
func TestLiveCleanUpOnlyCleansUpDomainWithSpecifiedKey(t *testing.T) {
	if !envTest.IsLiveTest() {
		t.Skip("skipping live test")
	}
	...
	err = provider.Present(envTest.GetDomain(), "", "abc123==")
	require.NoError(t, err)

	err = provider.Present(envTest.GetDomain(), "", "def456==")
	require.NoError(t, err)

	txtRecords, err := provider.findTxtRecords(envTest.GetDomain(), "")
	require.NoError(t, err)

	require.Len(t, txtRecords, 2)

	err = provider.CleanUp(envTest.GetDomain(), "", "def456==")
	require.NoError(t, err)
	...
	err = provider.CleanUp(envTest.GetDomain(), "", "abc123==")
	require.NoError(t, err)

	txtRecords, err = provider.findTxtRecords(envTest.GetDomain(), "")
	require.NoError(t, err)

	require.Len(t, txtRecords, 0)
}
